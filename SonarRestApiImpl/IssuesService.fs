...
let getIssueFromString(responsecontent : string) =
    ...
    issue.CreationDate <- data.CreationDate.DateTime
    ...
    issue.UpdateDate <- data.UpdateDate.DateTime
    ...
    if not(obj.ReferenceEquals(data.JsonValue.TryGetProperty("comments"), null)) then
        for elemC in data.Comments do issue.Comments.Add(new Comment(elemC.CreatedAt.DateTime.Value.DateTime, elemC.HtmlText, elemC.Key, elemC.Login, -1))

    if not(obj.ReferenceEquals(data.JsonValue.TryGetProperty("closeDate"), null)) then
        issue.CloseDate <- DateTime.Parse(data.CloseDate)
...
let getIssuesFromStringAfter45(responsecontent : string) =
    ...
    for elem in data.Issues do
        ...
        issue.CreationDate <- elem.CreationDate.DateTime

        issue.Component <- elem.Component
        if elem.Line.IsSome then
            issue.Line <- elem.Line.Value
        else
            issue.Line <- 0

        if elem.TextRange.IsSome then
            let textRange = TextRange()
            textRange.StartLine <- elem.TextRange.Value.StartLine
            textRange.EndLine <- elem.TextRange.Value.EndLine
            textRange.StartOffset <- elem.TextRange.Value.StartOffset
            textRange.EndOffset <- elem.TextRange.Value.EndOffset
            issue.TextRange <- textRange
            if issue.Line = 0 then
                issue.Line <- textRange.StartLine

        issue.Project <- elem.Project
        issue.UpdateDate <- elem.UpdateDate.DateTime
        ...
        if not(obj.ReferenceEquals(elem.Comments, null)) then
            for elemC in elem.Comments do issue.Comments.Add(new Comment(elemC.CreatedAt.DateTime, elemC.HtmlText, elemC.Key, elemC.Login, -1))

        if not(obj.ReferenceEquals(elem.CloseDate, null)) then
            match elem.CloseDate with
            | None -> ()
            | Some value -> issue.CloseDate <- DateTime.Parse(value)
...
let getIssuesFromString(responsecontent : string) =
    ...
    for elem in data.Issues do
        try
            let issue = new Issue()
            issue.Message <- elem.Message
            issue.CreationDate <- elem.CreationDate.DateTime.Value.DateTime
            ...
            issue.Project <- elem.Project
            issue.UpdateDate <- elem.UpdateDate.DateTime.Value.DateTime
            issue.Status <- (EnumHelper.asEnum<IssueStatus>(elem.Status)).Value
            issue.Severity <- GetSeverity(elem.Severity)
            issue.Rule <- elem.Rule
            ...
            if not(obj.ReferenceEquals(elem.CloseDate, null)) then
                issue.CloseDate <- DateTime.Parse(elem.CloseDate.Value)
...
let getViolationsFromString(responsecontent : string, reviewAsIssues : System.Collections.Generic.List<Issue>) =
    ...
    for elem in data do
        let issue = new Issue()
        issue.CreationDate <- elem.CreatedAt.DateTime.Value.DateTime
...
let getReviewsFromString(responsecontent : string) =
    ...
    for elem in data do
        ...
        issue.UpdateDate <- elem.UpdatedAt.DateTime.Value.DateTime
        issue.Status <- (EnumHelper.asEnum<IssueStatus>(elem.Status)).Value
        issue.Severity <- GetSeverity(elem.Severity)
        issue.Id <- elem.Id
        issue.Rule <- ""
        ...
        let Comments = elem.JsonValue.TryGetProperty("comments")
        if Comments.IsSome then
            for elemC in elem.Comments do issue.Comments.Add(new Comment(elemC.UpdatedAt.DateTime, elemC.Text, "", elemC.Author, elem.Id))
        ...
        try
            ...
            for i = 2 to value do
                ...
                if not(token.IsCancellationRequested) then
                    try
                        let url = url + "&p=" + Convert.ToString(i)
...
let SetIssueTags(conf : ISonarConfiguration, issue : Issue, tags : System.Collections.Generic.List<string>, httpconnector:IHttpSonarConnector, token:CancellationToken, logger:IRestLogger) =
...
let GetAvailableTags(newConf : ISonarConfiguration, httpconnector:IHttpSonarConnector, token:CancellationToken, logger:IRestLogger) =
